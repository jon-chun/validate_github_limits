...
class GitHubValidator:
   ...
   def _handle_file_size(self, path: Path, size_mb: float) -> None:
       ...
       if size_mb > MAX_GITHUB_FILESIZE_MB:
           msg = f"ERROR: {path.relative_to(self.repo_dir)} is {size_mb:.1f}MB (max {MAX_GITHUB_FILESIZE_MB}MB)"
           self.issues.append(msg)
           logging.error(msg)
           if self.auto_move:
               self.move_large_file(path)
       elif size_mb > MAX_GITHUB_FILESIZE_WARNING_MB:
           msg = f"WARNING: {path.relative_to(self.repo_dir)} is {size_mb:.1f}MB"
           self.issues.append(msg)
           logging.warning(msg)
   ...
   def move_large_file(self, file_path: Path) -> None:
       """Relocate file to backup directory and create symlink.
       
       Args:
           file_path: Path to file requiring relocation
       """
       try:
           relative_path = file_path.relative_to(self.repo_dir)
           new_path = self.backup_dir / relative_path
           new_path.parent.mkdir(parents=True, exist_ok=True)
           
           shutil.move(str(file_path), str(new_path))
           file_path.symlink_to(new_path)
           
           msg = f"Moved {relative_path} to {new_path}"
           self.issues.append(msg)
           logging.info(msg)
       except Exception as e:
